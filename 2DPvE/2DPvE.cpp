...
void AIActive(Player &Ai, Player &Player) {
	...
	if (sqrt(max(pow(Ai.x - Player.x, 2) + pow(Ai.y - Player.y, 2), 0.1)) < 10) {
		if (Ai.FireDelay[2] == 0)
	...
	if (sqrt(max(pow(Ai.x - Player.x, 2) + pow(Ai.y - Player.y, 2), 0.1)) > 10 &&
		...
		approachAI(Ai.x, Ai.y, 10, Ai, Player);
		Ai.goOn = 5;
	}
	...
	if (sqrt(max(pow(Ai.x - Player.x, 2) + pow(Ai.y - Player.y, 2), 0.1)) < 10 && Ai.goOn == 0) {
